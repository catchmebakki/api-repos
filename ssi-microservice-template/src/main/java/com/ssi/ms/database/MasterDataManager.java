...
import com.ssi.ms.database.servcie.AllowValAlvService;
import com.ssi.ms.database.servcie.NhuisLogNhlService;
...
public class MasterDataManager {
    ...
    @Autowired
    private AllowValAlvService allowValAlvService;

    @Autowired
     private NhuisLogNhlService nhuisLogNhlService;

    public void getAllAlvValue(final Long id) {

        allowValAlvService.findAll()
            .forEach(allowValAlv -> {
                System.out.println(allowValAlv.getAlvId());
            });

        nhuisLogNhlService.save(this.createNhuisLog.get());
    }
